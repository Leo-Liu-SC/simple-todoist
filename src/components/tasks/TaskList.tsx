      ...
      <div className="px-6 py-4 border-b border-slate-200/80 flex items-center gap-3">
        ...
        <div className="flex items-center bg-slate-100 rounded-lg p-0.5">
          <button
            ...
            className={`p-1.5 rounded-md transition-all ${viewMode === "list" ? "bg-white shadow-sm text-slate-700" : "text-slate-500 hover:text-slate-600"}`}
            title="List view"
          >
            <ListIcon size={15} />
          </button>
          <button
            ...
            className={`p-1.5 rounded-md transition-all ${viewMode === "board" ? "bg-white shadow-sm text-slate-700" : "text-slate-500 hover:text-slate-600"}`}
            title="Board view"
          >
            <Columns3 size={15} />
          </button>
            ...
            {showSortMenu && (
              <>
                ...
                <div className="absolute right-0 top-9 z-20 bg-white border border-slate-200 rounded-xl shadow-[var(--shadow-pop)] py-2 w-60">
                  <p className="px-3 pb-1 text-[11px] font-semibold text-slate-500 uppercase tracking-wider">Sort</p>
                  {sortRules.length === 0 ? (
                    <p className="px-3 py-1 text-xs text-slate-500">Click a column header to sort. Click more headers to combine.</p>
                  ) : (
                    <div className="px-3 py-1 space-y-1">
                      {sortRules.map((r, i) => (
                        <div key={r.key} className="flex items-center gap-2 text-sm text-slate-600">
                          <span className="text-[11px] text-slate-500 w-3">{i + 1}</span>
                          <span className="flex-1 capitalize">{r.key === "dueDate" ? "Due date" : r.key}</span>
                          <span className="text-xs text-slate-500">{r.dir === "asc" ? "↑ asc" : "↓ desc"}</span>
                        </div>
                      ))}
        ...
        <div className="relative">
          <button
            ...
            className={`p-1.5 rounded-lg text-slate-500 hover:text-slate-700 hover:bg-slate-100 transition-colors ${viewMode === "board" ? "opacity-40 pointer-events-none" : ""}`}
          ...
          {showColumnMenu && (
            <>
              ...
              <div className="absolute right-0 top-9 z-20 bg-white border border-slate-200 rounded-xl shadow-[var(--shadow-pop)] py-2 w-48">
                <p className="px-3 pb-1.5 text-[13px] font-semibold text-slate-500 uppercase tracking-wider">Columns</p>
      ...
      <div className="flex-1 overflow-y-auto">
        ...
        {!isLoading && filtered.length === 0 && !showQuickAdd && (
          <div className="flex flex-col items-center justify-center py-20 gap-3 text-center">
            ...
            <p className="text-xs text-slate-500 -mt-1">Add one below to get started</p>
          </div>
        )}
        {canReorder ? (
        ...
        {showQuickAdd ? (
          ...
          <div className="px-4 py-3">
            <button
              ...
              className="flex items-center gap-2 w-full text-sm text-slate-500 hover:text-indigo-600 px-2 py-2 rounded-lg hover:bg-indigo-50/50 transition-colors"
            >
        ...
        {!showCompleted && completedCount > 0 && (
          <button
            onClick={toggleCompleted}
            className="mx-4 mb-3 text-xs text-slate-500 hover:text-slate-600 transition-colors"
          >
